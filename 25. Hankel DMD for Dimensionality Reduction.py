...
class HankelDMDTransformer(BaseEstimator, TransformerMixin):

    def __init__(self, tlsq_rank=0, exact=False,
                 opt=False, rescale_mode=None, forward_backward=False,
                 d=1, sorted_eigs=False, reconstruction_method='first',
    ...
    def fit(self, X, y=None):
        ...
        hdmd = HankelDMD(svd_rank=self.n_components, tlsq_rank=self.tlsq_rank, exact=self.exact,
                         opt=self.opt, rescale_mode=self.rescale_mode, forward_backward=self.forward_backward,
                         d=self.d, sorted_eigs=self.sorted_eigs, reconstruction_method=self.reconstruction_method,
                         tikhonov_regularization=self.tikhonov_regularization)
        fit = hdmd.fit(X.T)
        self.modes = fit.modes
        print(np.shape(self.modes))

    def transform(self, X, y=None):
        transformed_data = (X @ self.modes).real
...
hdmd = HankelDMDTransformer()
hdmd.fit(sc_data)

hdmd_data = hdmd.transform(sc_data)
print(np.shape(hdmd_data))
