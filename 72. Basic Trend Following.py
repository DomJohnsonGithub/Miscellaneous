...
for close_price in close:
    ...
    if ((apo > APO_VALUE_FOR_SELL_ENTRY and abs(
            ...
            or
            (position > 0 and (
                    apo <= 0 or open_pnl > MIN_PROFIT_TO_CLOSE))):  # long from +ve APO and APO has gone negative or position is profitable, sell to close position
    ...
    # 2. We are short( -ve position ) and either APO trading signal value is at or above 0 or current position is profitable enough to lock profit.
            ...
            close_price - last_buy_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE)  # APO above buy entry threshold, we should buy
          or
          (position < 0 and (
                  apo >= 0 or open_pnl > MIN_PROFIT_TO_CLOSE))):  # short from -ve APO and APO has gone positive or position is profitable, buy to close position
