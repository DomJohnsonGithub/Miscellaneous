...
# Strategy
# - includes an option to only trade on certain weekdays
class MLStrategy(bt.Strategy):
    ...
    def log(self, txt, dt=None):
        ...
        with Path(self.p.log_file).open("a") as f:
            ...
            log_writer.wrtierow([dt.isoformat()] + txt.split(","))
    ...
    def next(self):
        today = self.data[0].datetime.date()
        ...
        longs = sorted(up, key=up.get, reverse=True)[:self.n_positions]
