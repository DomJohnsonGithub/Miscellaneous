...
# Risk limits
"""
Risk limits, which we are not allowed to breach. 
We will be set to 150% of the historically observed maximums:
"""
...
"""
We will maintain some variables to track and check 
for risk violations:
"""
risk_violated = False
...
"""
Computing the Simple Moving Average and Standard Deviation for 
volatility adjustments. We will compute the fast and slow EMAs 
and the APO value, which we can use as our mean reversion trading
signal:
"""
close = data['Close']
for close_price in close:
    ...
    """
    Before we can evaluate our signal and check whether we can send an order
    ...
    if NUM_SHARES_PER_TRADE > RISK_LIMIT_MAX_TRADE_SIZE:
        print('RiskViolation NUM_SHARES_PER_TRADE', NUM_SHARES_PER_TRADE, ' > RISK_LIMIT_MAX_TRADE_SIZE',
    ...
    """
    Checks the trading signal to see if we should send orders as
    ...
    if (not risk_violated and
            ((apo > APO_VALUE_FOR_SELL_ENTRY * stdev_factor and abs(
                close_price - last_sell_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE * stdev_factor)  # APO above sell entry threshold, we should sell
             or
             (position > 0 and (
                     apo >= 0 or open_pnl > MIN_PROFIT_TO_CLOSE / stdev_factor)))):  # long from -ve APO and APO has gone positive or position is profitable, sell to close position
    ...
    # 2. We are short( -ve position ) and either APO trading signal value is at or below 0 or current position is profitable enough to lock profit.
          ...
          ((apo < APO_VALUE_FOR_BUY_ENTRY * stdev_factor and abs(
              close_price - last_buy_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE * stdev_factor)  # APO below buy entry threshold, we should buy
           or
           (position < 0 and (
                   apo <= 0 or open_pnl > MIN_PROFIT_TO_CLOSE / stdev_factor)))):  # short from +ve APO and APO has gone negative or position is profitable, buy to close position
    ...
    """
    Check that, after any potential orders have been sent out and trades
    have been made this round, we haven't breached any of our risk limits, starting
    with the Maximum Position Holding Time risk limit: 
    """
    ...
    """
    Check that the new long/short position is within the Max Position risk
    ...
    """
    Also, check that the updated traded volume doesn't violate the allocated
    ...
    """
    Code that updates the PnLs, unchanged from before:
    """
    open_pnl = 0
    if position > 0:
    ...
    if len(pnls) > 5:
        ...
        if weekly_loss < RISK_LIMIT_WEEKLY_STOP_LOSS:
            print('RiskViolation weekly_loss', weekly_loss, ' < RISK_LIMIT_WEEKLY_STOP_LOSS',
    ...
    if len(pnls) > 20:
        ...
        if monthly_loss < RISK_LIMIT_MONTHLY_STOP_LOSS:
            print('RiskViolation monthly_loss', monthly_loss, ' < RISK_LIMIT_MONTHLY_STOP_LOSS',
